...
class L0Module(Module):
    ...
    def constrain_parameters(self):
        # Constrain the parameters to be in a reasonable range
    ...
    def get_num_parameters_for_mlp(self):
        if hasattr(self, "intlayer_loga"):
            intlayer_score = torch.ones(self.num_hidden_layers)
        else:
    ...
    def get_num_parameters_and_constraint(self):
        ...
        if not hasattr(self, "intlayer_loga"):
            intlayer_score = torch.ones(self.mlp_num_per_layer)
        else:
            intlayer_score = 1 - self.cdf_qz(0, self.intlayer_loga)  # 12
        int_score = 1 - self.cdf_qz(0, self.int_loga)  # 12 * 3072
        intlayer_score = intlayer_score.unsqueeze(-1)
    ...
    def get_z_from_zs(self, zs):
        ...
        for type in self.all_types:
            ...
            elif name == "head_layer" or name == "mlp":
                z = np.ones(self.num_hidden_layers)
            elif name == "hidden":
                z = np.ones(self.hidden_size)
            else:
                raise ValueError("type {} not in types {}".format(name, self.types))
            if torch.is_tensor(z): 
                new_z = z.squeeze().detach().cpu().numpy() > 0
                numpified_zs[name] = new_z
            else:
                numpified_zs[name] = z>0
        return numpified_zs
    ...
    def forward(self, training=True,):
        ...
        if training:
            # for training, we sample z from concrete distribution
            for i, type in enumerate(self.types):
                loga = self.z_logas[type]
                z = self._sample_z(loga)
                zs[f"{type}_z"] = z.reshape(self.shapes[type])
        else:
            # for inference, we use deterministic z
            for i, type in enumerate(self.types):
                if type != "hidden": # hidden is not a per layer sample
                    loga_all_layers = self.z_logas[type] # load loga for all layers
...
if __name__ == "__main__":
    ...
    l0_module = L0Module(config, lagrangian_warmup=200, target_sparsity=0.5,pruning_type='structured_heads+structured_mlp+layer+hidden')
    zs= l0_module.forward(training=False)
    # print(zs.keys())  
    # print(zs['head_z'].size())  
    print(zs['intermediate_z'].size())  
    print(zs['intermediate_z'])  
    # print(zs['mlp_z'].size())
    # print(zs['hidden_z'].size())
    # print(zs['head_layer_z'].size())
    # print(l0_module.lagrangian_regularization(150))  
    # print(l0_module.calculate_model_size(zs))   